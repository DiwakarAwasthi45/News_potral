...
import { news, sidebarNews,  } from "../Data";
        ...
        <main className="lg:col-span-3">
          ...
          {view === "grid" && (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {news.map((item) => (
                <Link href={`/news/${item.slug}`} key={item.id} className="bg-white shadow rounded-xl overflow-hidden hover:shadow-lg transition">
                  <img src={item.image} className="w-full h-52 object-cover" />
                  <div className="p-4">
                    <h2 className="font-bold text-lg">{item.title}</h2>
                    <p className="text-gray-600 text-sm mt-2">{item.desc}</p>
                               <Link className="text-[#6DA7E7]" href={`/news/${item.slug}`}>
  थप पढ्नुस् →
</Link>
                  </div>
                </Link>
          ...
          {view === "list" && (
            <div className="flex flex-col gap-4">
              {news.map((item) => (
                <Link href={`/news/${item.slug}`} key={item.id} className="flex gap-4 bg-white shadow rounded-xl p-3">
